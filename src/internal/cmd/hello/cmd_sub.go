...
type SubCmdConfig struct {
	...
	valueRepeatPrintout clihelpers.IntOptConf
}
...
func NewSubCmdConfig(cmd *cli.Cmd) *SubCmdConfig {
	...
	config.valueRepeatPrintout.Value = cmd.Int(clihelpers.NewCliIntOpt("repeat r", 1, "Define how often message should be printed: 1-10", &config.valueRepeatPrintout.SetByUser))
	return &config
}
...
func DoBeforeSubCmd(app *cli.Cli, appConfig *AppConfig, cmd *cli.Cmd, config *SubCmdConfig) {
	...
	if !clihelpers.CliValidateIntValue(*config.valueRepeatPrintout.Value, 1, 10) {
		clihelpers.CliExitWithErr(app, cmd, 1, clihelpers.CliIntValueRangeErr("--repeat", *config.valueRepeatPrintout.Value, 1, 10), true)
	}
}
...
func DoSubCmd(app *cli.Cli, appConfig *AppConfig, cmd *cli.Cmd, config *SubCmdConfig) {
	...
	for i := *config.valueRepeatPrintout.Value; i > 0; i-- {
		if *appConfig.flagShowWelcome.Value {
			fmt.Printf("%s\n", *appConfig.welcomeText.Value)
		}
	}
